...
export class RelayerSDKLoader {
  ...
  public load(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onload = () => {
        // Poll for window.relayerSDK to be ready (max 5 seconds)
        ...
        const checkReady = () => {
          ...
          if (isFhevmWindowType(window, this._trace)) {
            console.log(`[RelayerSDKLoader] script onload SUCCESS! (took ${attempts * 100}ms)`);
            resolve();
          } else if (attempts >= maxAttempts) {
            console.log("[RelayerSDKLoader] script onload FAILED after timeout...");
            reject(
              new Error(
                `RelayerSDKLoader: Relayer SDK script has been successfully loaded from ${SDK_CDN_URL}, however, the window.relayerSDK object is invalid after ${maxAttempts * 100}ms.`
              )
            );
          } else {
            // Try again in 100ms
